...
/// Ice_Axe
#[macro_export]
macro_rules! helper__weapon {
    ($ctx:expr, $world:expr) => {{
        $ctx.has(Item::Ice_Axe)
    }};
}
#[macro_export]
macro_rules! hexplain__weapon {
...
/// $melee (  )
/// $weapon or ^mode == 'drone'
#[macro_export]
macro_rules! helper__melee {
    ($ctx:expr, $world:expr) => {{
        (helper__weapon!($ctx, $world) || $ctx.mode() == enums::Mode::Drone)
    }};
}
#[macro_export]
...
macro_rules! helper__offset {
    ($ctx:expr, $world:expr) => {{
        ($world.major_glitches && helper__weapon!($ctx, $world))
    }};
}
#[macro_export]
...
macro_rules! hobserve__offset {
    ($ctx:expr, $world:expr, $full_obs:expr) => {{
        ($world.major_glitches && (hobserve__weapon!($ctx, $world, $full_obs)))
    }};
}
...
/// $melee_cskip (  )
/// ($weapon and indra_cskip) or ^mode == 'drone'
#[macro_export]
macro_rules! helper__melee_cskip {
    ($ctx:expr, $world:expr) => {{
        ((helper__weapon!($ctx, $world) && $world.indra_cskip) || $ctx.mode() == enums::Mode::Drone)
    }};
}
#[macro_export]
...
macro_rules! hobserve__melee_cskip {
    ($ctx:expr, $world:expr, $full_obs:expr) => {{
        ((hobserve__weapon!($ctx, $world, $full_obs) && ($world.indra_cskip)) || {
            let v = {
                $full_obs.observe_mode();
                $ctx.mode()
            };
    ...
    ($ctx:expr, $world:expr, $edict:expr) => {{
        {
            ...
            let mut h = {
                let h = $ctx.has(Item::Heretics_Granddaughter);
                $edict.insert("Heretics_Granddaughter", format!("{}", h));
                (h, vec!["Heretics_Granddaughter"])
            };
            refs.append(&mut h.1);
            if !h.0 {
                return (false, refs);
            ...
            let mut h = {
                let h = $ctx.has(Item::Urgent_Message);
                $edict.insert("Urgent_Message", format!("{}", h));
                (h, vec!["Urgent_Message"])
            };
            refs.append(&mut h.1);
            (h.0, refs)
        }
...
macro_rules! hobserve__all_notes {
    ($ctx:expr, $world:expr, $full_obs:expr) => {{
        ({
            ...
            $full_obs.observe_urgent_message();
            $ctx.has(Item::Urgent_Message)
        })
    }};
}
...
/// $all_flasks (  )
/// [Flask{53}, Big_Flask{30}]
#[macro_export]
macro_rules! helper__all_flasks {
    ($ctx:expr, $world:expr) => {{
        $ctx.count(Item::Flask) >= 53 && $ctx.count(Item::Big_Flask) >= 30
    }};
}
#[macro_export]
...
macro_rules! hobserve__all_flasks {
    ($ctx:expr, $world:expr, $full_obs:expr) => {{
        ({
            $full_obs.observe_flask(IntegerObservation::Ge(53));
            $ctx.count(Item::Flask) >= 53
        }) && ({
...
/// $all_health (  )
/// [Health_Node{5}, Health_Fragment{24}]
#[macro_export]
macro_rules! helper__all_health {
    ($ctx:expr, $world:expr) => {{
        $ctx.count(Item::Health_Node) >= 5 && $ctx.count(Item::Health_Fragment) >= 24
    }};
}
#[macro_export]
    ...
    ($ctx:expr, $world:expr, $edict:expr) => {{
        {
            ...
            let mut h = {
                ...
                (ct >= 24, vec!["Health_Fragment count"])
            };
            refs.append(&mut h.1);
            (h.0, refs)
        }
...
macro_rules! hobserve__all_health {
    ($ctx:expr, $world:expr, $full_obs:expr) => {{
        ({
            ...
            $full_obs.observe_health_fragment(IntegerObservation::Ge(24));
            $ctx.count(Item::Health_Fragment) >= 24
        })
    }};
}
    ...
    ($ctx:expr, $world:expr, $edict:expr) => {{
        {
            ...
            let mut h = {
                let h = $ctx.has(Item::Diviners_Gem);
                $edict.insert("Diviners_Gem", format!("{}", h));
                (h, vec!["Diviners_Gem"])
            };
            refs.append(&mut h.1);
            if !h.0 {
            ...
            let mut h = {
                let h = $ctx.has(Item::Halusan);
                $edict.insert("Halusan", format!("{}", h));
                (h, vec!["Halusan"])
            };
            refs.append(&mut h.1);
            if !h.0 {
...
/// $all_urns (  )
/// [Amashilama, Anuman, Breach_Sight, Drone_Hover, Drone_Melee_Charge, Exit_Breach, Fast_Travel, Infect, Ledge_Grab,  Melee_Charge, Nanite_Mist, Remote_Drone, Shockwave, Slingshot_Charge, Slingshot_Hook, Slingshot_Weapon,   Underwater_Movement, Wall_Climb]
#[macro_export]
